...
import com.dunk.tfc.api.Events.ItemCookEvent;
...
public class TEFirepit extends TEFireEntity implements IInventory
{
	...
	public TEFirepit()
	{
		...
		fireItemStacks = new ItemStack[12];
		hasCookingPot = true;
		ashNumber = 1;
	}
		...
		if (fireItemStacks[1] != null)
		{
			...
			if (index != null && TFC_ItemHeat.getTemp(fireItemStacks[1]) > index.meltTemp)
			{
				...
				if (output != null)
				{
					...
					{
						// if both output slots are empty then just lower the
						// input item into the first output slot
				...
				// Check if we should combine the output with a pre-existing
				// output
				if (output != null && output.getItem() instanceof ItemMeltedMetal)
				{
							...
							.getItem() && fireItemStacks[7].getItemDamage() > 0)
					{
						...
						leftover = amt3 - 100;// assign the leftover so that we
												// can add to the other slot if
												// applicable
						if (leftover > 0)
							addLeftover = true;
						int amt4 = 100 - amt3;// convert the percent back to mc
												// damage
						if (amt4 < 0)
					...
					if (addLeftover)
					{
								...
								.getItem() && fireItemStacks[dest].getItemDamage() > 0)
						{
							int amt1 = 100 - leftover;// the percentage of the
														// output
							int amt2 = 100 - fireItemStacks[dest].getItemDamage();// the
																					// percentage
																					// currently
																					// in
																					// the
																					...
																					// slot
							int amt3 = amt1 + amt2;// combined amount
							int amt4 = 100 - amt3;// convert the percent back to
													// mc damage
							if (amt4 < 0)
								amt4 = 0;// stop the infinite glitch
			...
			else if (fireItemStacks[1] != null && fireItemStacks[1].getItem() instanceof ItemClothing)
			{
				TFC_Core.handleQuickDry(fireItemStacks[1]);
			}
		}
	}
	...
	public void updateEntity()
	{
		if (!worldObj.isRemote)
		{
			...
			if (list != null && !list.isEmpty() && fireItemStacks[0] == null) // Only
																				// go
																				// through
																				// the
																				// list
																				...
																				// fit.
			{
				...
				for (Iterator iterator = list.iterator(); iterator.hasNext();)
				{
					...
					if (item == TFCItems.logs || item == Item.getItemFromBlock(TFCBlocks.peat))
					{
						...
						{
							if (fireItemStacks[0] == null) // Secondary check
															// for empty input
															// slot.
							{
								/**
								 * Place a copy of only one of the logs into the
								 * fuel slot, due to the stack limitation of the
								 * fuel slots. Do not change to
								 * fireItemStacks[0] = is;
								 */
								setInventorySlotContents(0, new ItemStack(item, 1, is.getItemDamage()));
								is.stackSize--;
								handleFuelStack(); // Attempt to shift the fuel
													// down so more fuel can be
													// added within the same for
													// loop.
							}
						}
			...
			if (fireItemStacks[1] != null && fireItemStacks[1]
				...
				((ItemPotterySmallVessel) fireItemStacks[1].getItem()).onUpdate(fireItemStacks[1], worldObj, xCoord,
						yCoord, zCoord);
			...
			/*
			 * hasCookingPot = fireItemStacks[1] != null
			 * &&fireItemStacks[1].getItem() == TFCItems.PotteryPot &&
	...
	private void smokeBlock(int x, int y, int z)
	{
				...
				.getTileEntity(x, y, z) instanceof TESmokeRack)
		{
			...
			for (int i = 0; i < te.storage.length; i++)
			{
				...
				if (is != null)
				{
					if (Food.getSmokeCounter(is) < Food.SMOKEHOURS) // Smoking
																	// in
																	// progress
					{
						// Smoking does not make up for lost time to balance
						// fuel consumption
					...
					{
						Food.setFuelProfile(is, EnumFuelMaterial.getFuelProfile(fuelTasteProfile));
						broadcast = true; // Broadcast change to update string
											// color
					}
				}
			}
	...
	public void readFromNBT(NBTTagCompound nbttagcompound)
	{
		...
		ashNumber = nbttagcompound.getInteger("ash");
	...
	public void writeToNBT(NBTTagCompound nbttagcompound)
	{
		...
		nbttagcompound.setInteger("ash", ashNumber);
	...
	{
		/*
		 * if(scanSmokeLayer) { int y = 0; int topY = yCoord; boolean
		 * checkArray[][][] = new boolean[25][13][25];
		 * while(worldObj.getBlock(xCoord, topY + 1, zCoord) ==
		 * TFCBlocks.LogPile) { //scanLogs(xCoord, topY, zCoord, checkArray,
		 * (byte)12, (byte)y, (byte)12, false, true); ++topY; ++y; }
		 * //scanLogs(xCoord, topY, zCoord, checkArray, (byte)12, (byte)y,
		 * (byte)12, false, true); smokeMap.put(smokeMap.size(), new int[]
		 * {xCoord, topY + 2, zCoord}); scanSmokeLayer = false; }
		 * 
		 * if(smokeMap.size() > 0) { int[] sb =
		 * smokeMap.get(worldObj.rand.nextInt(smokeMap.size())); float f = sb[0]
		 * + 0.5F; float f1 = sb[1] + 0.1F + worldObj.rand.nextFloat() * 6F /
		 * 16F; float f2 = sb[2] + 0.5F; float f4 = worldObj.rand.nextFloat() *
		 * 0.6F; float f5 = worldObj.rand.nextFloat() * -0.6F;
		 * worldObj.spawnParticle("smoke", f+f4 - 0.3F, f1, f2 + f5 + 0.3F,
		 * 0.0D, 0.0D, 0.0D); worldObj.spawnParticle("smoke", f+f4 - 0.2F, f1,
		 * f2 + f5 + 0.4F, 0.0D, 0.0D, 0.0D); worldObj.spawnParticle("smoke",
		 * f+f4 - 0.1F, f1, f2 + f5 + 0.1F, 0.0D, 0.0D, 0.0D);
		 * if(worldObj.rand.nextInt(10) == 0)
		 * worldObj.spawnParticle("largesmoke", f+f4 - 0.2F, f1, f2 + f5 + 0.2F,
		 * 0.0D, 0.0D, 0.0D); }
		 */
	}
}
