...
public class TECrop extends NetworkTileEntity
{
	...
	private int killLevel;// We use this to make crop killing more and more
							// likely if its cold
	...
	public void updateEntity()
	{
		...
		if (!worldObj.isRemote)
		{
			...
			// ChunkData cd = TFC_Core.getCDM(worldObj).getData(xCoord >> 4,
			// zCoord >> 4);
			if (crop != null && growthTimer < time && sunLevel > 0)
			{
				...
				float ambientTemp = TFC_Climate.getHeightAdjustedTempSpecificDay(worldObj,
						TFC_Time.getDayOfYearFromTick(growthTimer), xCoord, yCoord, zCoord);
				float tempAdded = 0;
				...
				float growthRate = Math.max(0.0f,
						((((crop.numGrowthStages / (crop.growthTime * TFC_Time.timeRatio96)) * nutriMult)) * 3f * TFCOptions.cropGrowthMultiplier) + tempAdded);
				...
				// config setting
				if ((TFCOptions.enableCropsDie || !TFC_Core.isFarmland(worldObj.getBlock(xCoord, yCoord - 1,
						zCoord))) && (crop.maxLifespan == -1 && growth > crop.numGrowthStages + ((float) crop.numGrowthStages / 2)) || growth < 0)
				{
					killCrop(crop);
				}
	...
	public boolean activateCrop(World world, int x, int y, int z, EntityPlayer player, CropIndex crop)
	{
		// Check again that this crop can be activated
		if (crop != null && crop.activateHarvestable && world.getBlock(x, y - 1,
				z) instanceof BlockFarmland && growth >= crop.numGrowthStages)
		{
			...
			if (is1 != null)
				worldObj.spawnEntityInWorld(new EntityItem(world, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, is1));

			if (is2 != null)
				worldObj.spawnEntityInWorld(new EntityItem(world, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, is2));
			...
			EntityItem ei = new EntityItem(world, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, seedStack);
			...
			float growthRate = Math.max(0.0f,
					((((crop.numGrowthStages / (crop.growthTime * TFC_Time.timeRatio96)) * nutriMult)) * 3f * TFCOptions.cropGrowthMultiplier));
			
			growth = crop.numGrowthStages - (0 + (float)Math.max(growthRate,0.01));
			growthTimer += TFC_Time.DAY_LENGTH * crop.regrowMultiplier;
	...
	public void onHarvest(World world, EntityPlayer player, boolean isBreaking)
	{
		if (!world.isRemote)
		{
			...
			if (crop.requiresLadder && world.getBlock(xCoord, yCoord - 1, zCoord) instanceof BlockFarmland)
			{
