...
public class TEForge extends TEFireEntity implements IInventory
{
	...
	/*
	 * private class CoordDirection { int x; int y; int z; ForgeDirection dir;
	 * public CoordDirection(int x, int y, int z, ForgeDirection d) { this.x =
	 * x;this.y = y;this.z = z;this.dir = d; } }
	 */
	...
	public void cookItem(int i)
	{
		...
		if (fireItemStacks[i] != null)
		{
			...
			if (index != null && TFC_ItemHeat.getTemp(fireItemStacks[i]) > index.meltTemp)
			{
				...
				// If not unshaped metal, morph the input to the output. If not
				// an input with direct morph (sand, sticks, etc) this deletes
				// the input item from the slot.
				if (!(fireItemStacks[i].getItem() instanceof ItemMeltedMetal)
						&& !(fireItemStacks[i].getItem() instanceof ItemPotterySheetMold
								&& fireItemStacks[i].getItemDamage() >= 2))
				...
				if (fireItemStacks[i] != null)
				{
					...
					if (morphIndex != null)
					{
						// Apply old temperature to direct morphs that can
				...
				else if (index.hasOutput())
				{
					...
					if (inputCopy.getItem() instanceof ISmeltable && (inputCopy.getItem() instanceof ItemTerra)
							&& ((ItemTerra) inputCopy.getItem()).isActualMetal(inputCopy))
					{
						ISmeltable smelt = (ISmeltable) inputCopy.getItem();
						// ItemStack meltedItem = new
						// ItemStack(smelt.getMetalType(inputCopy).meltedItem);
						// TFC_ItemHeat.setTemp(meltedItem, temperature);
						int units = smelt.getMetalReturnAmount(inputCopy);
						// Raw/Refined Blooms give at max 100 units to force
						// players to split using the anvil
						if (inputCopy.getItem() instanceof ItemBloom)
							units = Math.min(100, units);

						while (units > 0 && getMold(smelt.getMetalType(inputCopy)) != null)
						{
							ItemStack moldIS = this.getMold(smelt.getMetalType(inputCopy));
							// Create a new item based on the proper output of
							// melting this metal into this mold.
							ItemStack outputCopy = new ItemStack(
									smelt.getMetalType(inputCopy).getResultFromMold(moldIS.getItem()));// meltedItem.copy();
							outputCopy.setItemDamage(
									smelt.getMetalType(inputCopy).getBaseValueForResult(moldIS.getItem()));
							...
							((ItemPotteryMoldBase) (outputCopy.getItem())).addUnits(outputCopy,
									((ItemPotteryMoldBase) (moldIS.getItem())).getUnits(moldIS));
							// int maxUnits =
							// ((ItemPotteryMoldBase)(moldIS.getItem())).getMaxUnits();
							int possibleUnits = ((ItemPotteryMoldBase) (outputCopy.getItem()))
									.getPossibleUnits(outputCopy);
							if (units > possibleUnits)
							{
								units -= possibleUnits;
								outputCopy = ((ItemPotteryMoldBase) (outputCopy.getItem())).setFullUnits(outputCopy);
								moldIS.stackSize--;
								updateStorageSlots();
								if (!addToStorage(outputCopy.copy()))
								{
									EntityItem ei = new EntityItem(worldObj, xCoord + 0.5, yCoord + 1.5, zCoord + 0.5,
											outputCopy);
									ei.motionX = 0;
									ei.motionY = 0;
									ei.motionZ = 0;
							...
							else if (units > 0) // Put the last item in the
												// forge cooking slot, replacing
												// the input.
							{
								((ItemPotteryMoldBase) (outputCopy.getItem())).addUnits(outputCopy, units);// outputCopy.setItemDamage(100-units);
								units = 0;
								moldIS.stackSize--;
								updateStorageSlots();
					...
					{
						// if the input is a new item, then apply the old
						// temperature to it
						TFC_ItemHeat.setTemp(fireItemStacks[i], temperature);
					}
				}
	...
	//If the stack size is 0, delete the item.
	public void updateStorageSlots()
	{
		for (int i = 10; i < 14; i++)
		{
			if (fireItemStacks[i] != null && fireItemStacks[i].stackSize == 0)
			{
	...
	private ItemStack getMold(Metal m)
	{
		if (fireItemStacks[10] != null && fireItemStacks[10].getItem() instanceof ItemPotteryMoldBase
				&& ((ItemPotteryMoldBase) (fireItemStacks[10].getItem())).isValidMold(fireItemStacks[10])
				&& m.isValidMold(fireItemStacks[10]) && fireItemStacks[10].stackSize > 0)
		{
			return fireItemStacks[10];
		}
		else if (fireItemStacks[11] != null && fireItemStacks[11].getItem() instanceof ItemPotteryMoldBase
				&& ((ItemPotteryMoldBase) (fireItemStacks[11].getItem())).isValidMold(fireItemStacks[11])
				&& m.isValidMold(fireItemStacks[11]) && fireItemStacks[11].stackSize > 0)
		{
			return fireItemStacks[11];
		}
		else if (fireItemStacks[12] != null && fireItemStacks[12].getItem() instanceof ItemPotteryMoldBase
				&& ((ItemPotteryMoldBase) (fireItemStacks[12].getItem())).isValidMold(fireItemStacks[12])
				&& m.isValidMold(fireItemStacks[12]) && fireItemStacks[12].stackSize > 0)
		{
			return fireItemStacks[12];
		}
		else if (fireItemStacks[13] != null && fireItemStacks[13].getItem() instanceof ItemPotteryMoldBase
				&& ((ItemPotteryMoldBase) (fireItemStacks[13].getItem())).isValidMold(fireItemStacks[13])
				&& m.isValidMold(fireItemStacks[13]) && fireItemStacks[13].stackSize > 0)
		{
	...
	{
		/*
		 * if (fireItemStacks[10] != null && fireItemStacks[10].getItem() ==
		 * TFCItems.ceramicMold) return 10; if (fireItemStacks[11] != null &&
		 * fireItemStacks[11].getItem() == TFCItems.ceramicMold) return 11; if
		 * (fireItemStacks[12] != null && fireItemStacks[12].getItem() ==
		 * TFCItems.ceramicMold) return 12; if (fireItemStacks[13] != null &&
		 * fireItemStacks[13].getItem() == TFCItems.ceramicMold) return 13;
		 */
		return -1;
	}
	...
	public void updateEntity()
	{
		...
		if (!worldObj.isRemote)
		{
			...
			if (r.nextInt(10) == 0 && fireTemp > 20)
				worldObj.playSoundEffect(xCoord, yCoord, zCoord, "fire.fire", 0.4F + (r.nextFloat() / 2),
						0.7F + r.nextFloat());
			...
			if (fuelTimeLeft > 0 && fireTemp >= 1 && !TFC_Core.isExposedToRain(worldObj, xCoord, yCoord, zCoord))
			{
				...
				TFC_Core.handleItemTicking(fuelStack, worldObj, xCoord, yCoord, zCoord, false);
			}
			else if (fuelTimeLeft <= 0 && fireTemp >= 1 && fireItemStacks[7] != null && isSmokeStackValid)
			{
				// here we set the temp and burn time based on the fuel in the
				// bottom slot.
