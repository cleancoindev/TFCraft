...
import com.dunk.tfc.api.Interfaces.IMergeableFood;
...
public class ItemFoodTFC extends ItemTerra implements ISize, ICookableFood, IMergeableFood
{
	...
	{
		//if(canStack())
			//return 64;//return this.getSize(null).stackSize * getWeight(null).multiplier <= 64 ? this.getSize(null).stackSize * getWeight(null).multiplier : 64;
			//else
				return 1;
	}
	...
	public static ItemStack createTag(ItemStack is, float weight)
	{
		...
		f.add(new float[]{weight,-24});
		Food.setFoodsInStack(is, f);
		//Food.setDecay(is, -24);
	...
	public IFood setSmokeAbsorbMultiplier(float s) {
		smokeAbsorb = s;
		return this;
	}
}
