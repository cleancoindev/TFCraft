...
public class TFC_Time
{
	...
	public static float getPercentSummer(long days, int z)
	{
		//The year starts in March. Summer is July. 
		int day = getDayOfYearFromDays(days);
		int summerDay = (int)((double)daysInMonth*4.5d);
		return z <=0?1f - ((float)Math.abs(summerDay - day)/(float)daysInYear):((float)Math.abs(summerDay - day)/(float)daysInYear);		
	}
	
	public static float getPercentWinter(long days, int z)
	{
		return 1f - getPercentSummer(days,z);
	}
