...
import com.dunk.tfc.api.SkillsManager.Skill;
import com.dunk.tfc.api.Events.GetSkillMultiplierEvent;
import com.dunk.tfc.api.Events.PlayerSkillEvent;
...
public class SkillStats
{
	...
	public float getSkillMultiplier(String skillName)
	{
		...
		//If we're not comfortable, our skill is poor.
		int disc = TFC_Core.getBodyTempStats(player).discomfort;
		BodyTempStats s = TFC_Core.getBodyTempStats(player);
		if (disc >= 2 || TFC_Core.getBodyTempStats(player).discomfort <= -2)
		{
			return Math.min(0.249f, event.skillResult);
		}
