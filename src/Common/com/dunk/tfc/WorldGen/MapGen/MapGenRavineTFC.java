package com.dunk.tfc.WorldGen.MapGen;
		...
		for (; par15 < par16; ++par15)
		{
			...
			if (var54 || var19.nextInt(4) != 0)
			{
				...
				if (xCoord >= chunkMidX - 16.0D - var53 * 2.0D && zCoord >= chunkMidZ - 16.0D - var53 * 2.0D && xCoord <= chunkMidX + 16.0D + var53 * 2.0D && zCoord <= chunkMidZ + 16.0D + var53 * 2.0D)
				{
					...
					int minY = height > Global.SEALEVEL?Global.SEALEVEL+2:MathHelper.floor_double(yCoord - var30) - 1;
					int maxY = yCoord > Global.SEALEVEL?254:MathHelper.floor_double(yCoord + var30) + 1;
