		...
		if(rand.nextInt(30) == 0 && j <= 150)
		{
			for (int xCoord = i - radius; xCoord <= i + radius; ++xCoord)
			{
				for (int zCoord = k - radius; zCoord <= k + radius; ++zCoord)
				{
					int x = xCoord - i;
					int z = zCoord - k;
					if (x * x + z * z <= radius * radius && (TFC_Climate.getRainfall(world, xCoord, Global.SEALEVEL, zCoord) >= 400 || world.getBiomeGenForCoords(i, k).biomeID == TFCBiome.RIVER.biomeID))
					{
						flag = false;
