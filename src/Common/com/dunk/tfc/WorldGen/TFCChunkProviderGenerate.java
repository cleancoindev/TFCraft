...
import com.dunk.tfc.WorldGen.MapGen.MapGenCavesTFC;
import com.dunk.tfc.WorldGen.MapGen.MapGenRavineTFC;
import com.dunk.tfc.WorldGen.MapGen.MapGenRiverRavine;
...
public class TFCChunkProviderGenerate extends ChunkProviderGenerate
{
	...
	private MapGenRavineTFC surfaceRavineGen = new MapGenRavineTFC(175, 30);// surface
	...
	public Chunk provideChunk(int chunkX, int chunkZ)
	{
		...
		biomesForGeneration = this.worldObj.getWorldChunkManager().loadBlockGeneratorData(biomesForGeneration,
				chunkX * 16 - 1, chunkZ * 16 - 1, 18, 18);
		if (TFC_Climate.getCacheManager(worldObj) != null)
		{
			rockLayer1 = TFC_Climate.getCacheManager(worldObj).loadRockLayerGeneratorData(rockLayer1, chunkX * 16,
					chunkZ * 16, 16, 16, 0);
			rockLayer2 = TFC_Climate.getCacheManager(worldObj).loadRockLayerGeneratorData(rockLayer2, chunkX * 16,
					chunkZ * 16, 16, 16, 1);
			rockLayer3 = TFC_Climate.getCacheManager(worldObj).loadRockLayerGeneratorData(rockLayer3, chunkX * 16,
					chunkZ * 16, 16, 16, 2);
			evtLayer = TFC_Climate.getCacheManager(worldObj).loadEVTLayerGeneratorData(evtLayer, chunkX * 16,
					chunkZ * 16, 16, 16);
			rainfallLayer = TFC_Climate.getCacheManager(worldObj).loadRainfallLayerGeneratorData(rainfallLayer,
					chunkX * 16, chunkZ * 16, 16, 16);
			stabilityLayer = TFC_Climate.getCacheManager(worldObj).loadStabilityLayerGeneratorData(stabilityLayer,
					chunkX * 16, chunkZ * 16, 16, 16);
			drainageLayer = TFC_Climate.getCacheManager(worldObj).loadDrainageLayerGeneratorData(drainageLayer,
					chunkX * 16, chunkZ * 16, 16, 16);
		}
	...
	public static List<SpawnListEntry> getCreatureSpawnsByChunk(World world, TFCBiome biome, int x, int z)
	{
		...
		if (temp > -20 && temp <= 0)
		{
			...
			if (rain > 250)
			{
				...
				spawnableCreatureList.add(new SpawnListEntry(EntityWolfTFC.class, 2 + mountainousAreaModifier, 1,
		...
		{
			spawnableCreatureList.add(new SpawnListEntry(EntityPigTFC.class, 2 + mountainousAreaModifier,
					2 + mountainousAreaModifier, 4 + mountainousAreaModifier));
			spawnableCreatureList.add(new SpawnListEntry(EntityChickenTFC.class, 3 + mountainousAreaModifier, 1,
	...
	public void generateTerrainHigh(int chunkX, int chunkZ, Block[] idsTop)
	{
		...
		this.biomesForGeneration = this.worldObj.getWorldChunkManager().getBiomesForGeneration(this.biomesForGeneration,
				chunkX * 4 - 2, chunkZ * 4 - 2, xSize + 5, zSize + 5);
		this.noiseArray = this.initializeNoiseFieldHigh(this.noiseArray, chunkX * subDivXZ, 0, chunkZ * subDivXZ, xSize,
				ySize, zSize);
		...
		if (this.parabolicField == null)
		{
			...
			for (int counter1 = -2; counter1 <= 2; ++counter1)
			{
				for (int counter2 = -2; counter2 <= 2; ++counter2)
				{
					...
					// http://i.imgur.com/rxrui67.png
				}
			}
		}

		/*
		 * Mojang Magic Numbers double var44 = 684.412D; double var45 =
		 * 684.412D;
		 */

		double double1 = 1000D;
		double double2 = 1000D;
		// double1 and double 2 are only used in these generateNoiseOctaves
		// methods.

		// this.noise5 = this.noiseGen5.generateNoiseOctaves(this.noise5, xPos,
		// zPos, xSize, zSize, 1.121D, 1.121D, 0.5D);
		this.noise6 = this.noiseGen6.generateNoiseOctaves(this.noise6, xPos, zPos, xSize, zSize, 200.0D, 200.0D, 0.5D);
		this.noise3 = this.noiseGen3.generateNoiseOctaves(this.noise3, xPos, yPos, zPos, xSize, ySize, zSize,
				double1 / 80.0D, double2 / 160.0D, double1 / 80.0D);
		// this.noise3 = this.noiseGen3.generateNoiseOctaves(this.noise3, xPos,
		// yPos, zPos, xSize, ySize, zSize, var44 / 80.0D, 0.5, var44 / 80.0D);
		this.noise1 = this.noiseGen1.generateNoiseOctaves(this.noise1, xPos, yPos, zPos, xSize, ySize, zSize, double1,
				double2, double1);
		...
		for (int x = 0; x < xSize; ++x)
		{
			for (int z = 0; z < zSize; ++z)
			{
				...
				if (scaledNoise6Value < 0.0D)
					scaledNoise6Value = -scaledNoise6Value * 0.3D; // If
																	// negative,
																	// make
																	// positive
																	// and
				...
				if (scaledNoise6Value < 0.0D) // Only true when noise6[index2]
												// is between -17,777 and 0,
												// scaledNoise6Value will be at
												// maximum -2
				{
					scaledNoise6Value /= 2.0D; // Results in values between 0
												// and -1
					if (scaledNoise6Value < -1.0D) // Error Checking
						scaledNoise6Value = -1.0D;
					scaledNoise6Value /= 1.4D * 2.0D; // Results in values
														// between 0 and
														// -0.357143
				}
				else
				{
					if (scaledNoise6Value > 1.0D)
						scaledNoise6Value = 1.0D;
					scaledNoise6Value /= 8.0D; // Results in values between 0
												// and 0.125
				}
			...
			byte[] metaBig)
	{
		...
		stoneNoise = noiseGen4.generateNoiseOctaves(stoneNoise, chunkX * 16, chunkZ * 16, 0, 16, 16, 1, var6 * 4.0D,
				var6 * 1.0D, var6 * 4.0D);
		...
		for (int xCoord = 0; xCoord < 16; ++xCoord)
		{
			...
			for (int zCoord = 0; zCoord < 16; ++zCoord)
			{
				...
				byte alternateSurfaceMeta = (byte) TFC_Core.getSoilMeta(rock1.data1);
				byte alternateSubsurfaceMeta = (byte) TFC_Core.getSoilMeta(rock1.data1);
				...
				for (int height = 127; height >= 0; --height)
				{
					...
					// metaBig[indexBig] = 0;
					float temp = TFC_Climate.adjustHeightToTemp(height + 128, bioTemp);
					...
					if (idsBig[indexBig] == Blocks.stone)
					{
						...
						else if (rain <= 125 && biome.heightVariation < 0.5f && temp > 16f)
						{
							// We want deserts to be silica sand now.
							// surfaceBlock =
							// TFC_Core.getTypeForSand(rock1.data1);
							surfaceBlock = TFC_Core.getTypeForSand(15); // Quartzite
																		// sand
																		// on
																		// the
																		// top
						...
						if (biome == TFCBiome.BEACH || biome == TFCBiome.OCEAN || biome == TFCBiome.DEEP_OCEAN)
						{
							if (bioTemp > 25 && biome != TFCBiome.DEEP_OCEAN)
							{
								subSurfaceBlock = surfaceBlock = TFC_Core.getTypeForSand(10); // Chalk
																								// sand
								alternateSurfaceMeta = alternateSubsurfaceMeta = 10;
							}
							else if (bioTemp <= 25 && bioTemp > 10 && biome != TFCBiome.DEEP_OCEAN)
							{
								subSurfaceBlock = surfaceBlock = TFC_Core.getTypeForSand(15); // Quartzite
																								// sand
								alternateSurfaceMeta = alternateSubsurfaceMeta = 15;
							}
							else
							{
						...
						if (var13 == -1)
						{
							// The following makes dirt behave nicer and more
							// smoothly, instead of forming sharp cliffs.
							...
							if (var13 > 0)
							{
											 ...
											 */ dirtH > 0)
								{
									...
									{
										idsBig[indexBig] = TFC_Core.getTypeForDirt(alternateSubsurfaceMeta);
										metaBig[indexBig] = alternateSubsurfaceMeta; // (byte)TFC_Core.getSoilMeta(rock1.data1);
									}
								...
								|| height < seaLevel && idsTop[index + 1] == TFCBlocks.saltWaterStationary)
						{
																...
																// sand bottoms
							{
								...
								{
									// idsBig[indexBig] =
									// TFC_Core.getTypeForGravel(rock1.data1);
									// Make it so that the gravel is the same
									// type as the sand?
									// metaBig[indexBig] = (byte)
									// TFC_Core.getSoilMeta(rock1.data1);
									// metaBig[indexBig] = (byte)
									// alternateSubsurfaceMeta;
									if (bioTemp > 25 && biome != TFCBiome.DEEP_OCEAN)
									{
										idsBig[indexBig] = TFC_Core.getTypeForGravel(10); // Chalk
																							// sand
										metaBig[indexBig] = 10;
									}
									else if (bioTemp <= 25 && bioTemp > 10 && biome != TFCBiome.DEEP_OCEAN)
									{
										idsBig[indexBig] = TFC_Core.getTypeForGravel(15); // Quartzite
																							// sand
										metaBig[indexBig] = 15;
									}
									else
							...
							else // Swamp biomes have bottoms that are mostly
									// dirt
							{
		...
		/*
		 * for (int x = 0; x < 16; ++x) { for (int z = 0; z < 16; ++z) { int
		 * arrayIndex = getIndex(x, z); int arrayIndex2 = getIndex(x+1, z+1);
		 * 
		 * } }
		 */
	}
	...
	/*
	 * private int getIndex(int x, int z) { return x + z * 16; }
	 */
	...
	private void replaceBlocksForBiomeLow(int par1, int par2, Random rand, Block[] idsBig, byte[] metaBig)
	{
		...
		for (int xCoord = 0; xCoord < 16; ++xCoord)
		{
			for (int zCoord = 0; zCoord < 16; ++zCoord)
			{
				...
				TFCBiome biome = (TFCBiome) getBiome(xCoord, zCoord);
				float bioTemp = TFC_Climate.getBioTemperature(worldObj, par1 * 16 + xCoord, par2 * 16 + zCoord);
				for (int height = 127; height >= 0; --height)
				{
					// int index = ((arrayIndex) * 128 + height);
					...
					else if (idsBig[indexBig] == null)
					{
						...
						if (TFC_Core.isBeachBiome(biome.biomeID) || TFC_Core.isOceanicBiome(biome.biomeID))
						{
							...
							{
								// Not tropical
								if (bioTemp <= 25 && bioTemp > 5)
								{
									idsBig[indexBig] = TFC_Core.getTypeForSand(15);
									metaBig[indexBig] = (byte) TFC_Core.getSoilMeta(15);
									idsBig[indexBig - 1] = TFC_Core.getTypeForSand(15);
									metaBig[indexBig - 1] = (byte) TFC_Core.getSoilMeta(15);
								}
								else if (bioTemp > 25)
								{
									idsBig[indexBig] = TFC_Core.getTypeForSand(10);
									metaBig[indexBig] = (byte) TFC_Core.getSoilMeta(10);
									idsBig[indexBig - 1] = TFC_Core.getTypeForSand(10);
									metaBig[indexBig - 1] = (byte) TFC_Core.getSoilMeta(10);
								}
								else
								{
