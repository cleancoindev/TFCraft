...
public class EntityFishHookTFC extends EntityFishHook
{
	...
	public static int getMaxTension()
	{
		return MAX_LINE_TENSION;
	}
	...
	public Vec3 applyEntityForce(Vec3 entityForce, double x, double y, double z){
		...
		lineTension += (forceRatio -31 > 1 ? Math.sqrt(forceRatio - 31) : forceRatio - 31)*50;
		...
		if(is != null && is.getItem() instanceof ItemCustomFishingRod){
			...
			if(this.reelCounter > 2){
				is.stackTagCompound.setInteger("tension", lineTension/*(int) (((forceRatio)-29) + (Math.pow(entityForce.lengthVector() / 0.2,3)*2))*100*/);
			}
			else{
				this.reelCounter++;
			}
		...
		if(lineTension >= MAX_LINE_TENSION){
			...
			((EntityFishTFC)(this.ridingEntity)).setFree();
	...
	private void onHit(Entity e)
	{
		if(this.ridingEntity == null)
		{
			...
			TFC_Core.sendInfoMessage(this.field_146042_b, new ChatComponentTranslation("fishingRod.bite"));
		}
	}

	//Used to generate fish randomly, now we want to catch real fish
	...
	public void attemptToCatch(){
		...
		if(!this.isRiding())
		{
			if( hitTick == 0)
			{
				for(EntityFishTFC f : (List<EntityFishTFC>)(this.worldObj.getEntitiesWithinAABB(EntityFishTFC.class, this.boundingBox.expand(0.5, 0.75, 0.5))))
				{
					if(hitCount < rand.nextInt(3)+3)
					{
						hitTick = 45;
						hitCount++;
						this.onHit(f);
	...
	public void reelInBobber(Entity entity, ItemStack itemstack){
		if(hitTick > 35 && hitCount > rand.nextInt(3))
		{
			func_146034_e();
			hitTick = 0;
		}
		...
		{
			for(EntityFishTFC f : (List<EntityFishTFC>)(this.worldObj.getEntitiesWithinAABB(EntityFishTFC.class, this.boundingBox.expand(0, 0.5, 0))))
			{
				f.setScared(this);
			}
	...
	public int func_146034_e/*catchFish*/()
	{
		...
		{
			EntityPlayer player = this.field_146042_b;
			EntityFishTFC fish = (EntityFishTFC)biter;//new EntityFishTFC(this.worldObj);
