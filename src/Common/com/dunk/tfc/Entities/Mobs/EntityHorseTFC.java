...
public class EntityHorseTFC extends EntityHorse implements IInvBasic, IAnimal
{
	...
	private boolean creativeFamiliarizer;
	...
	protected void dropFewItems(boolean par1, int par2)
	{
		...
		this.dropItem(TFCItems.bone, (int) ((rand.nextInt(8) + 3) * ageMod));
		this.dropItem(TFCItems.sinew, (int) ((rand.nextInt(6) + 2) * ageMod));
		if(this.getLeashed()){
	...
	public void familiarize(EntityPlayer ep) {
		...
		if ((this.riddenByEntity == null || !this.riddenByEntity.equals(ep)) && !familiarizedToday && stack != null && this.isFood(stack) && canFamiliarize())
		{
			...
			if(ep.capabilities.isCreativeMode)
			{
				this.creativeFamiliarizer = true;
			}
			else
			{
				this.creativeFamiliarizer = false;
			}
	...
	public void handleFamiliarityUpdate() {
		...
		if(lastFamiliarityUpdate < totalDays){
			if(familiarizedToday && familiarity < 100){
				...
				float familiarityChange = (this.creativeFamiliarizer?10:1)*6 * obedienceMod / aggressionMod;
	...
	public void readEntityFromNBT(NBTTagCompound nbttc)
	{
		...
		if (nbttc.hasKey("SaddleItem", 10))
		{
			...
			if (itemstack != null && itemstack.getItem() == TFCItems.saddleTFC)
			{
		...
		{
			this.horseChest.setInventorySlotContents(0, new ItemStack(TFCItems.saddleTFC));
		}
