...
public class ContainerPlayerTFC extends ContainerPlayer
{
	...
	public ItemStack slotClick(int sourceSlotID, int destSlotID, int clickType, EntityPlayer p)
	{
		if (sourceSlotID >= 0 && sourceSlotID < this.inventorySlots.size())
		{
			...
			// case.
			else if (clickType == 1 && sourceSlotID == 0 && isInventoryFull() && slotStack != null
					&& slotStack.getItem() instanceof IFood)
				return null;
		}
	//	if(sourceSlotID == -999)
		//{
	...
	private boolean canMergeAnyway(ItemStack is, ItemStack is2)
	{
		Item i1 = is.getItem();
		Item i2 = is2.getItem();
		if ((i1 instanceof IFood && i2 instanceof IFood) && i1 == i2)
		{
			return true;
		}
		return false;
			...
			boolean craftingOutput)
	{
		...
		if (is.isStackable())
		{
					...
					&& (!reverseOrder && slotIndex < slotFinish || reverseOrder && slotIndex >= slotStart))
			{
				...
				// && !is.getHasSubtypes()
						&& is.getItemDamage() == slotstack.getItemDamage() && canMergeAnyway(is, slotstack)
