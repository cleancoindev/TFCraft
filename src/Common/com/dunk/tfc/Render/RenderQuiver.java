...
import com.dunk.tfc.Render.Models.ModelQuiver;
	...
	public void makeAdjustments(EntityLivingBase entity, float partialRenderTick)
	{
		try
        {
            ...
            if (entity.isRiding() && entity.ridingEntity instanceof EntityLivingBase)
            {
                ...
                f2 = this.interpolateRotation(entitylivingbase1.prevRenderYawOffset, entitylivingbase1.renderYawOffset, partialRenderTick);
            ...
            float f13 = entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialRenderTick;
        ...
        //    this.preRenderCallback(entity, partialRenderTick);
            ...
            float f6 = entity.prevLimbSwingAmount + (entity.limbSwingAmount - entity.prevLimbSwingAmount) * partialRenderTick;
           ...
           // this.mainModel.setLivingAnimations(entity, f7, f6, partialRenderTick);
           // this.renderModel(entity, f7, f6, f4, f3 - f2, f13, f5);
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                if (j > 0)
                {
         //           this.renderPassModel.setLivingAnimations(entity, f7, f6, partialRenderTick);
        //            this.renderPassModel.render(entity, f7, f6, f4, f3 - f2, f13, f5);
          ...
          //              this.renderPassModel.render(entity, f7, f6, f4, f3 - f2, f13, f5);
           ...
           /* float f14 = entity.getBrightness(partialRenderTick);
            j = this.getColorMultiplier(entity, f14, partialRenderTick);
            ...
            if ((j >> 24 & 255) > 0 || entity.hurtTime > 0 || entity.deathTime > 0)
            {
                ...
                if (entity.hurtTime > 0 || entity.deathTime > 0)
                {
                    ...
                    for (int l = 0; l < 4; ++l)
                    {
                        if (this.inheritRenderPass(entity, l, partialRenderTick) >= 0)
                        {
                            GL11.glColor4f(f14, 0.0F, 0.0F, 0.4F);
                            this.renderPassModel.render(entity, f7, f6, f4, f3 - f2, f13, f5);
                        }
                ...
                if ((j >> 24 & 255) > 0)
                {
                    ...
                    {
                        if (this.inheritRenderPass(entity, i1, partialRenderTick) >= 0)
                        {
                            GL11.glColor4f(f8, f9, f15, f10);
                            this.renderPassModel.render(entity, f7, f6, f4, f3 - f2, f13, f5);
                        }
...
*/
	...
	public void doRender(EntityLivingBase entity, ItemStack item, float partialRenderTick){
		...
		{
			GL11.glTranslated(-interpolatePositions(cp.prevPosX,cp.posX,partialRenderTick), -interpolatePositions(cp.prevPosY,cp.posY,partialRenderTick), -interpolatePositions(cp.prevPosZ,cp.posZ,partialRenderTick));
			//makeAdjustments(entity, partialRenderTick);
		}
