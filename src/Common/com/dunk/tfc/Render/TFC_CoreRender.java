...
public class TFC_CoreRender
{
	...
	private static boolean isSnow(IBlockAccess access, int x, int y, int z)
	{
		...
		return material == Material.snow || material == Material.craftedSnow
				|| (access.getBlock(x, y, z).equals(TFCBlocks.worldItem)
						&& ((TEWorldItem) (access.getTileEntity(x, y, z))).snowLevel > 0);
	}
	...
	public static boolean renderSnow(Block block, int i, int j, int k, RenderBlocks renderblocks)
	{
		...
		if (isSnow(renderblocks.blockAccess, i, j, k))
		{
			...
			{
				//We want to render something to make it stand out as something else.
			...
			{
				extra = (isSnow(renderblocks.blockAccess, i + 1, j, k - 1)
						&& isSnow(renderblocks.blockAccess, i + 1, j, k));
			...
			{
				extra = (isSnow(renderblocks.blockAccess, i + 1, j, k + 1)
						&& isSnow(renderblocks.blockAccess, i, j, k + 1));
	...
	public static boolean renderWoodTrunk(Block block, int i, int j, int k, RenderBlocks renderblocks)
	{
		 ...
		 * blockAccess.getBlock(i, j-1, k) == mod_TFC_Core.fruitTreeWood)
		 */
		if (blockAccess.getTileEntity(i, j, k) != null && (blockAccess.getBlock(i, j - 1, k) == TFCBlocks.fruitTreeWood
