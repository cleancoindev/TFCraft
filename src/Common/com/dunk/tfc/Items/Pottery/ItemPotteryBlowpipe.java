...
public class ItemPotteryBlowpipe extends ItemPotteryMold 
{
			...
			/*	int dist = 0;
				...
				if(world.getTileEntity(x, y, z) instanceof TEFireEntity)
				{
					...
					if(te.canBlowWithPipe())
					{
						...
						world.playSoundEffect(entityplayer.posX, entityplayer.posY , entityplayer.posZ, TFC_Sounds.BELLOWS, 0.4F, 1);
						return true;
					}
				}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
	{
		if(itemstack.getItemDamage() == 2 && !world.isRemote)
		{
			...
			float f2 = rand.nextFloat() * 0.8F + 0.1F;
					...
					entityitem = new EntityItem(world, entityplayer.posX + f, entityplayer.posY + f1, entityplayer.posZ + f2, new ItemStack(TFCItems.glassBottle,1));
					entityitem.motionX = (float)rand.nextGaussian() * f3;
					entityitem.motionY = (float)rand.nextGaussian() * f3 + 0.2F;
					entityitem.motionZ = (float)rand.nextGaussian() * f3;
					world.spawnEntityInWorld(entityitem);
					itemstack.setItemDamage(1);
					world.playSoundEffect(entityplayer.posX + f, entityplayer.posY + f1, entityplayer.posZ + f2, TFC_Sounds.BELLOWS, 0.4F, 1);
		}
		return itemstack;
	}
	...
	public IIcon getIconFromDamage(int damage)
	{
		...
		else if(damage >= 2) return this.glassIcon;
