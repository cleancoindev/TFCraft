...
public class ItemLogs extends ItemTerra
{
	public ItemLogs()
	{
		...
		this.metaNames = new String[Global.WOOD_ALL.length*2];
		for(int i =0; i < Global.WOOD_ALL.length; i++)
		{
			metaNames[2*i] = Global.WOOD_ALL[i];
			metaNames[2*i+1] = Global.WOOD_ALL[i] + " Chopped";
		}
		this.setWeight(EnumWeight.MEDIUM);
		this.setSize(EnumSize.MEDIUM);
	}

	//Adding functionality for chopped logs
	@Override
	...
	{
		for(int i = 0; i < Global.WOOD_ALL.length; i++) {
			icons[2*i] = registerer.registerIcon(Reference.MOD_ID + ":" + "wood/"+Global.WOOD_ALL[i]+" Log");
			icons[2*i +1] = registerer.registerIcon(Reference.MOD_ID + ":" + "wood/"+Global.WOOD_ALL[i]+" Chopped Log");
		}
	}
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		if(!world.isRemote)
		{
			...
			else if(world.getBlock(x, y, z) == TFCBlocks.logPile)
			{
				...
				if(te != null)
				{
					//Helps fill log piles faster
					if(te.storage[0] != null && te.contentsMatch(0, itemstack) && te.getNumberOfLogs() == 1 && itemstack.stackSize == 15)
					{
						te.injectContents(0, 3);
						te.addContents(1, new ItemStack(this,4, itemstack.getItemDamage()));
						te.addContents(2, new ItemStack(this,4, itemstack.getItemDamage()));
						te.addContents(3, new ItemStack(this,4, itemstack.getItemDamage()));
						playSound(world, x, y, z);
					...
					else
					{
						...
						{
							counter = te.storage[i] != null ? i: -1;
						}
						if(counter != -1)
						{

							int prevStacksize = 0;
							while(itemstack.stackSize > 0 && prevStacksize != itemstack.stackSize)
							{
								prevStacksize = itemstack.stackSize;
								if(te.storage[0] == null)
								{
									te.storage[0] = new ItemStack(itemstack.getItem(),1,itemstack.getItemDamage());
									itemstack.stackSize--;
								}
								else if(te.storage[0].stackSize < 4 && te.contentsSame(0,itemstack))
								{
									te.storage[0].stackSize++;
									itemstack.stackSize--;
								}
								else if(te.storage[1] == null)
								{
									te.storage[1] = new ItemStack(itemstack.getItem(),1,itemstack.getItemDamage());
									itemstack.stackSize--;
								}
								else if(te.storage[1].stackSize < 4  && te.contentsSame(1,itemstack))
								{
									te.storage[1].stackSize++;
									itemstack.stackSize--;
								}
								else if(te.storage[2] == null)
								{
									te.storage[2] = new ItemStack(itemstack.getItem(),1,itemstack.getItemDamage());
									itemstack.stackSize--;
								}
								else if(te.storage[2].stackSize < 4 && te.contentsSame(2,itemstack))
								{
									te.storage[2].stackSize++;
									itemstack.stackSize--;
								}
								else if(te.storage[3] == null)
								{
									te.storage[3] = new ItemStack(itemstack.getItem(),1,itemstack.getItemDamage());
									itemstack.stackSize--;
								}
								else if(te.storage[3].stackSize < 4  && te.contentsSame(3,itemstack))
								{
									te.storage[3].stackSize++;
									itemstack.stackSize--;
								}
							}
						}
