...
public class BlockBranch extends BlockTerra
{
	...
	public void updateTick(World world, int x, int y, int z, Random rand)
	{
		if(!world.isRemote)
		{
			...
			//Find the direction we're coming from and check that block
		/*	if(!(world.getBlock(x + sourceX, y + sourceY, z + sourceZ) instanceof BlockBranch || world.getBlock(x + sourceX, y + sourceY, z + sourceZ) instanceof BlockLogNatural || 
					(sourceY == -1 && sourceX == 0 && sourceZ == 0 && 
					(world.getBlock(x + sourceX, y + sourceY, z + sourceZ) instanceof BlockGrass || world.getBlock(x + sourceX, y + sourceY, z + sourceZ) instanceof BlockDirt))))
			{
				world.setBlock(x, y, z, Blocks.air, 0, 0x2);
			}*/
		}
	...
	public Block getSourceBlock(IBlockAccess world, int x, int y, int z)
	{
	...
	public int getDistanceToTrunk(IBlockAccess world, int x, int y, int z)
	{
		Block b = getSourceBlock(world, x, y ,z);
		if(b instanceof BlockBranch)
		{
			return ((BlockBranch) b).getDistanceToTrunk(world, x + sourceX, y + sourceY, z+sourceZ) + 1;
		}
		return 1;
	}
	...
	{
		return world.blockExists(x, y, z) && (world.getBlock(x, y, z) != this && world.getBlock(x,y,z) != TFCBlocks.logNatural);
	}
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block block)
	{
		...
		if(!check)
		{
			...
			dropBlockAsItem(world, x, y, z, new ItemStack(TFCItems.logs, 1, damageDropped(meta)));
		}
	}
	
	protected void scanLogs(World world, int i, int j, int k, int meta, boolean[][][] checkArray, byte x, byte y, byte z, ItemStack stack)
	{
		if(y >= 0 && j + y < 256)
		{
