...
public class BlockCustomTallGrass extends BlockTallGrass implements IShearable
{
	...
	public void updateTick(World w, int x, int y, int z, Random rand)
	{
		...
		if(below != null){
			if(TFC_Core.isGrass(below) && !TFC_Core.isDryGrass(below) && w.getBlockMetadata(x, y, z) == 2)	//Short dry grass
			{
				w.setBlockMetadataWithNotify(x,y,z,0,2);
			}
			else if(TFC_Core.isDryGrass(below) && w.getBlockMetadata(x, y, z) == 0)	//Tall grass
			{
