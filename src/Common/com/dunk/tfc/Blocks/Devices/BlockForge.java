...
public class BlockForge extends BlockTerraContainer implements IHeatSource
{
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float hitX, float hitY, float hitZ)
	{
		if (!world.isRemote && world.getTileEntity(i, j, k) instanceof TEForge)
		{
			...
			if (equippedItem != null)
			{
				...
				else if(item instanceof ItemPotteryBlowpipe && equippedItem.getItemDamage()==1 && entityplayer.isSneaking())
				{
					return false;
				}
			}
