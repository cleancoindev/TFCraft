...
import com.dunk.tfc.WorldGen.Generators.WorldGenSaplings;
...
public class BlockGrass extends BlockTerra
{
	...
	public IIcon getIcon(IBlockAccess access, int x, int y, int z, int side)
	{
		...
		/*
		boolean surrounded = isSurroundedBySnow(access,x,y,z);
		...
		{
			if (TFCOptions.enableBetterGrass && TFC_Core.isGrass(access.getBlock(x, y - 1, z - 1)))
				return (surrounded)? Blocks.snow.getBlockTextureFromSide(0) : grassTopTexture;
		}
		else if (side == 3) //+Z
		{
			if (TFCOptions.enableBetterGrass && TFC_Core.isGrass(access.getBlock(x, y - 1, z + 1)))
	...
	private boolean isSurroundedBySnow(IBlockAccess access, int x, int y, int z)
	{
		boolean flag = isSnow(access, x, y+1, z);
		if(!(isSnow(access, x, y, z + 1)|| (access.getBlock(x, y, z + 1)).isBlockSolid(access,x, y, z + 1,3)))flag = false;
		if(!(isSnow(access, x, y, z - 1)|| (access.getBlock(x, y, z - 1)).isBlockSolid(access,x, y, z - 1,2)))flag = false;
		if(!(isSnow(access, x + 1, y, z)|| (access.getBlock(x + 1, y, z)).isBlockSolid(access,x + 1, y, z,4)))flag = false;
		if(!(isSnow(access, x - 1, y, z)|| (access.getBlock(x - 1, y, z)).isBlockSolid(access,x - 1, y, z,5)))flag = false;
	...
	{
		return /*isSurroundedBySnow(bAccess,x,y,z) ? 0xffffff :*/ TerraFirmaCraft.proxy.grassColorMultiplier(bAccess, x, y, z);
	}
				...
				if(j < 250 && j >= Global.SEALEVEL-1 &&  TFC_Climate.getHeightAdjustedTemp(world, i, j, k) > 0){
					for(int y = 0; y < 2 && !nearWater; y++)
					{
						for(int x = -4; x < 5 && !nearWater; x++)
						{
							...
							{
								if (world.blockExists(i + x, j - y, k + z) && world.getBlock(i + x, j - y, k + z).getMaterial() == Material.water)
									nearWater = true;
							}
						}
					}
