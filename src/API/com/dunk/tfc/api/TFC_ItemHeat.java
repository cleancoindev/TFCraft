...
public class TFC_ItemHeat 
{
	...
	public static boolean vesselInFirepit(HeatIndex hi)
	{
		if(hi.getOutputItem() == null)
		{
			return false;
		}
		return hi.getOutputItem().equals(TFCItems.powder) || hi.getOutputItem().equals(TFCItems.lime);
	}
	...
	public static float getSpecificHeat(ItemStack is)
	{
		...
		if(is != null && manager!=null)
		{
			...
			else if(is.getItem().equals(TFCItems.potterySmallVessel))
			{
				return 0.7f;
			}
			else
	...
	public static float isCookable(Metal m)
	{
		...
		if(manager!=null)
		{
			HeatIndex hi = manager.findMatchingIndex(new ItemStack(m.getResultFromMold(TFCItems.ceramicMold)));
			if(hi != null)
				return hi.meltTemp;
			else
				return -1;
