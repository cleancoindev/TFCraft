...
public class CraftingManagerTFC
{
	...
	private CraftingManagerTFC()
	{
		...
		// StringBuilder().append(recipes.size()).append("
		// recipes").toString());
	}
	...
	public ShapedRecipesTFC addRecipe(ItemStack itemstack,EnumRegion[]regions, Object aobj[])
	{
		String s = "";
		int i = 0;
		int j = 0;
		...
		ShapedRecipesTFC shapedRecipesTFC = new ShapedRecipesTFC(j, k, regions,aitemstack, itemstack);
		recipes.add(shapedRecipesTFC);
		return shapedRecipesTFC;
	}

	public ShapelessRecipesTFC addShapelessRecipe(ItemStack itemstack,EnumRegion[]regions, Object aobj[])
	{
		ArrayList<ItemStack> arraylist = new ArrayList<ItemStack>();
		Object aobj1[] = aobj;
		int i = aobj1.length;
		...
		ShapelessRecipesTFC recipesTFC = new ShapelessRecipesTFC(itemstack,regions, arraylist);
		recipes.add(recipesTFC);
		return recipesTFC;
	}

	public ItemStack findMatchingRecipe(InventoryCrafting inventorycrafting, int x, int y, int z, World world)
	{
		/*
		 * int i = 0; ItemStack itemstack = null; ItemStack itemstack1 = null;
		 * for (int j = 0; j < inventorycrafting.getSizeInventory(); j++) {
		 * ItemStack itemstack2 = inventorycrafting.getStackInSlot(j); if
		 * (itemstack2 == null) { continue; } if (i == 0) { itemstack =
		 * itemstack2; } if (i == 1) { itemstack1 = itemstack2; } i++; }
		 * 
		 * if (i == 2 && itemstack.getItem() == itemstack1.getItem() &&
		 * itemstack.stackSize == 1 && itemstack1.stackSize == 1 &&
		 * itemstack.getItem().isRepairable()) { // Item item =
		 ...
		 * (item.getMaxDamage() * 10) / 100; // int k1 = item.getMaxDamage() -
		 * j1; // if (k1 < 0) // { // k1 = 0; // } // return new
		 * ItemStack(itemstack.itemID, 1, k1); }
		 */
		for (int k = 0; k < recipes.size(); k++)
		{
			IRecipe irecipe = recipes.get(k);
			if (irecipe instanceof IRegionalRecipe)
			{
				if (((IRegionalRecipe) irecipe).matches(inventorycrafting, x, y, z, world))
				{
